CREATE OR REPLACE FUNCTION %SCHEMA%.index_key_insert_trigger() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
    DECLARE
            boxId char(1);
...
ALTER FUNCTION %SCHEMA%.index_key_insert_trigger() OWNER TO %OWNER%;

DROP TRIGGER IF EXISTS index_key_trigger ON %SCHEMA%.index_key;
