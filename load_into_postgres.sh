...
function create-idindex-trigger {
  ...
  sed "s/%SCHEMA%/$IDINDEX_SCHEMA/g" configuration_scripts/create_idindex_trigger.sql | sed "s/%OWNER%/$IDINDEX_OWNER/g" | PGPASSWORD="${DB_PWD}" psql -p "${PORT}"  -h localhost -d "$IDINDEX_DATABASE" -U "$USER" -a
...
function ensure-idindex-schema-and-tables {
  ...
  PGPASSWORD="${DB_PWD}" psql -p "${PORT}" -d "${IDINDEX_DATABASE}"  -h localhost -U "${USER}" -e -c "CREATE SCHEMA IF NOT EXISTS ""${IDINDEX_SCHEMA}""; ALTER SCHEMA ${IDINDEX_SCHEMA} OWNER TO ${IDINDEX_OWNER};"
  PGPASSWORD="${DB_PWD}" psql -p "${PORT}" -d "${IDINDEX_DATABASE}"  -h localhost -U "${USER}" -e -c "CREATE TABLE IF NOT EXISTS ""${INDEX_KEY}"" (entityid uuid NOT NULL, value_id integer NOT NULL); ALTER TABLE ${INDEX_KEY} OWNER TO ${IDINDEX_OWNER};"
  PGPASSWORD="${DB_PWD}" psql -p "${PORT}"   -h localhost -U "${USER}" -e -c "CREATE TABLE IF NOT EXISTS ""${INDEX_VALUE}"" (value_id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 2147483647 CACHE 1 ), version bigint NOT NULL, branch uuid NOT NULL, dbid character varying(4) COLLATE pg_catalog.""default"", CONSTRAINT index_value_pkey PRIMARY KEY (value_id), CONSTRAINT index_value_version_branch_dbid_key UNIQUE (version, branch, dbid)); ALTER TABLE ${INDEX_VALUE} OWNER TO ${IDINDEX_OWNER};"
  PGPASSWORD="${DB_PWD}" psql -p "${PORT}"  -h localhost -d "${IDINDEX_DATABASE}" -U "${USER}" -e -c "GRANT USAGE ON SCHEMA  ""${IDINDEX_SCHEMA}"" TO cpp_ro; GRANT SELECT ON ALL TABLES IN SCHEMA ""${IDINDEX_SCHEMA}"" TO cpp_ro;"


  for N in {0..15}; do
    SUFFIX=$(printf '%x' "$N")
    PGPASSWORD="${DB_PWD}" psql -p "${PORT}"  -h localhost -d "${IDINDEX_DATABASE}" -U "${USER}" -e -c "CREATE TABLE IF NOT EXISTS ""${INDEX_KEY}""_""${SUFFIX}"" () INHERITS (""${IDINDEX_SCHEMA}"".index_key); ALTER TABLE ${INDEX_KEY}_${SUFFIX} OWNER TO ${IDINDEX_OWNER};"
  done
...
function create-idindex-indexes-after-loading {
  ...
  for N in {0..15}; do
    SUFFIX=$(printf '%x' "$N")
    PGPASSWORD="${DB_PWD}" psql -p "${PORT}" -d "${IDINDEX_DATABASE}"  -h localhost -U "${USER}" -e -c "CREATE INDEX entity_index_${SUFFIX} ON ${IDINDEX_SCHEMA}.index_key_${SUFFIX} USING btree (entityid);"
#    psql -d "${IDINDEX_DATABASE}" -U "${USER}" -e -c "CREATE INDEX index_key_${SUFFIX}_value_id_brin ON ${IDINDEX_SCHEMA}.index_key_${SUFFIX} USING brin (value_id) WITH (pages_per_range='128');"
  ...
  PGPASSWORD="${DB_PWD}" psql -p "$PORT"-d "${IDINDEX_DATABASE}"  -h localhost -U "${USER}" -e -c "CREATE UNIQUE INDEX pk_index_value ON ${IDINDEX_SCHEMA}.index_value USING btree (value_id);"
...
function drop-idindex-indexes-before-loading {
  ...
  for N in {0..15}; do
    SUFFIX=$(printf '%x' "$N")
    PGPASSWORD="${DB_PWD}" psql -p "$PORT"-d "${IDINDEX_DATABASE}"  -h localhost -U "${USER}" -e -c "DROP INDEX IF EXISTS ${IDINDEX_SCHEMA}.index_pkey_${SUFFIX};"
    PGPASSWORD="${DB_PWD}" psql -p "$PORT"-d "${IDINDEX_DATABASE}"  -h localhost -U "${USER}" -e -c "DROP INDEX IF EXISTS ${IDINDEX_SCHEMA}.entity_index_${SUFFIX};"
  ...
  PGPASSWORD="${DB_PWD}" psql -p "$PORT"-d "${IDINDEX_DATABASE}"  -h localhost -U "${USER}" -e -c "DROP INDEX IF EXISTS ${IDINDEX_SCHEMA}.pk_index_value;"
...
function load-file {
  ...
  if [ -f "${_FILE}" ]; then
    zcat "${_FILE}" | PGPASSWORD="${DB_PWD}" psql -p "${PORT}" -U "${_USER}" -h localhost -d "${_DATABASE}" -c "\COPY ""${_SCHEMA}"".""${_TABLE}"" FROM STDIN DELIMITER '|' CSV HEADER" >/dev/null && echo "[$(date +'%Y-%m-%d %H:%M:%S.%3N')][${_TABLE}] ${_FILE} OK!" >> "./output_$TABLE"
  else
